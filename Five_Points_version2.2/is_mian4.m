function [flag Type] = is_mian4(Step_Set1,Step_Set2)
...
for i = 1:15
    ...
    [adjacent adjacent_part] = is_adjacent(Step_Set1(temp,1),4);
    if  adjacent % (length(adjacent_part) >= 4) && 
        flag_temp = 1;
    ...
    if flag_temp == 1
        ...
        if  ( is_existed([sp(1) - 1  sp(2)],Step_Set2) || is_existed([ep(1) + 1  ep(2)],Step_Set2) ) && ...
           ~( is_existed([sp(1) - 1  sp(2)],Step_Set2) && is_existed([ep(1) + 1  ep(2)],Step_Set2) )
             flag = 1;
...
for i = 1:15
    ...
    [adjacent adjacent_part] = is_adjacent(Step_Set1(temp,2),4);
    if  adjacent % (length(adjacent_part) >= 4) && 
        flag_temp = 1;
    ...
    if flag_temp == 1
        ...
        if  ( is_existed([sp(1)  sp(2) - 1],Step_Set2) || is_existed([ep(1)  ep(2) + 1],Step_Set2) ) && ...
           ~( is_existed([sp(1)  sp(2) - 1],Step_Set2) && is_existed([ep(1)  ep(2) + 1],Step_Set2) )
             flag = 1;
...
for i = 1:length(x2y)
    flag_temp = 0;
    temp = find(x2y == x2y(i));
    [adjacent1 adjacent_part1] = is_adjacent(Step_Set1(temp,1),4);
    [dim, adjacent_part2] = is_adjacent(Step_Set1(temp,2),4);
    if  adjacent1 % (length(adjacent_part) >= 4) && 
        flag_temp = 1;
    ...
    if flag_temp == 1
        ...
        if  ( is_existed([sp(1) - 1  sp(2) - 1],Step_Set2) || is_existed([ep(1) + 1  ep(2) + 1],Step_Set2) ) && ...
           ~( is_existed([sp(1) - 1  sp(2) - 1],Step_Set2) && is_existed([ep(1) + 1  ep(2) + 1],Step_Set2) )
            flag = 1;
