function [flag Type] = is_mian3(Step_Set1,Step_Set2)
...
for i = 1:15
    ...
    if flag_temp == 1
        ...
        if (  is_existed([sp(1) - 1  sp(2)],Step_Set2) && ~is_existed([ep(1) + 1  ep(2)],Step_Set2) && ~is_existed([ep(1) + 2  ep(2)],Step_Set2) ) || ...
           ...
           (  is_existed([ep(1) + 2  ep(2)],Step_Set2) &&  is_existed([sp(1) - 2  sp(2)],Step_Set2) )
            flag = 1;
        end
    end
...
for i = 1:15
    ...
    if flag_temp == 1
        ...
        if (  is_existed([sp(1)  sp(2) - 1],Step_Set2) && ~is_existed([ep(1)  ep(2) + 1],Step_Set2) && ~is_existed([ep(1)  ep(2) + 2],Step_Set2) ) || ...
           ...
           (  is_existed([ep(1)  ep(2) + 2],Step_Set2) &&  is_existed([sp(1)  sp(2) - 2],Step_Set2) )
            flag = 1;
        end
    end
end
...
for i = 1:length(x2y)
    ...
    if flag_temp == 1
        ...
        if (  is_existed([sp(1) - 1  sp(2) - 1],Step_Set2) && ~is_existed([ep(1) + 1  ep(2) + 1],Step_Set2) && ~is_existed([ep(1) + 2  ep(2) + 2],Step_Set2) ) || ...
           ...
           (  is_existed([ep(1) + 2  ep(2) + 2],Step_Set2) &&  is_existed([sp(1) - 2  sp(2) - 2],Step_Set2) )
            flag = 1;
        end
    end
end
